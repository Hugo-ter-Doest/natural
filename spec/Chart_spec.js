...
var Chart = require('../lib/natural/parsers/Chart.js');
...
describe('Chart', function() {
  ...
  it('should add items to the chart', function() {
    ...
    expect(chart.fullParseItems('S', 'cykitem')).toEqual([]);
    chart.addItem(i4);
    expect(chart.getCompleteItemsFromTo(0, 6)).toEqual([i4]);
    expect(chart.fullParseItems('S', 'cykitem')).toEqual([i4]);
    expect(chart.nrOfItems()).toEqual(4);
  });


  it('should generate parse trees from the chart', function() {
    expect(chart.parseTrees('S', "cykitem")).toEqual(["S(S(NP,VP),S(NP,VP),S(NP,VP))"]);
  });
});
