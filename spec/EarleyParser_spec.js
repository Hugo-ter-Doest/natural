/*
    Unit test for the Earley parser
...
var EarleyParser = require('../lib/natural/parsers/EarleyParser');
...
describe('EarleyParser', function() {
  ...
  it('should parse a sentence', function() {
    ...
    taggedSentence.addTaggedWord('2', 'number');
    taggedSentence.addTaggedWord('+', 'plus');
    taggedSentence.addTaggedWord('3', 'number');

    var chart = parser.parse(taggedSentence);
    var parse_trees = chart.parseTrees(parser.grammar.getStartSymbol(), "earleyitem");
    expect(parse_trees).toBeArray(['E(E(number(2())),plus(+()),E(number(3())))']);

    // 2 + 3 * 4
    taggedSentence = new Sentence();
    taggedSentence.addTaggedWord('2', 'number');
    taggedSentence.addTaggedWord('+', 'plus');
    taggedSentence.addTaggedWord('3', 'number');
    taggedSentence.addTaggedWord('*', 'multiply');
    taggedSentence.addTaggedWord('4', 'number');
    chart = parser.parse(taggedSentence);
    parse_trees = chart.parseTrees(parser.grammar.getStartSymbol(), "earleyitem");
    parse_trees.sort();
