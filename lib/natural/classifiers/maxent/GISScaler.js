...
GISScaler.prototype.calculateMaxSumOfFeatures = function() {
  ...
  DEBUG && console.log("GISScaler:calculateMaxSumOfFeatures:maxSum is " + this.C);
...
GISScaler.prototype.addCorrectionFeature = function() {
  if (this.calculateMaxSumOfFeatures()) {
    DEBUG && console.log("GISScaler:addCorrectionFeature:C is " + this.C);
    var that = this;
    ...
    DEBUG && console.log("GISScaler:addCorrectionFeature:correctionFeature " + JSON.stringify(correctionFeature));
    this.featureSet.addFeature(correctionFeature);
  }
  else {
    DEBUG && console.log("Correction feature not needed");
  }
};
...
GISScaler.prototype.run = function(maxIterations, minImprovement) {
  ...
  DEBUG && console.log("Distribution created");
  p.prepare();
  DEBUG && console.log("Distribution prepared");
  ...
  do {
    DEBUG && console.log("Iteration " + this.iteration + " - Log likelihood of sample: " + likelihood + " - Entropy: " + p.entropy());
    for (var i = 0; i < this.featureSet.size(); i++) {
      ...
      DEBUG && console.log("Iteration " + this.iteration + " - Feature " + i);
      newAlpha[i] = p.alpha[i] * Math.pow(observedExpectation / expectationApprox, 1 / this.C);

      DEBUG && console.log("GISScaler.run: old alpha[" + i + "]: " + p.alpha[i]);
      DEBUG && console.log("GISScaler.run: new alpha[" + i + "]: " + newAlpha[i]);
    }
    ...
    //DEBUG && console.log("Iteration " + this.iteration + " - Old likelihood: " + likelihood + " - New likelihood: " + newLikelihood);
    DEBUG && console.log("Iteration " + this.iteration + " - Old KL: " + KLDistance + " - New KL: " + newKLDistance);
